<!--
  ...
  Rencana ke depan: ganti nama tampilan, ganti/upload foto profil,
...
<div style="max-width:560px;margin:0 auto;padding:24px 16px 0;min-height:70vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center">
  ...
  {:else if _me && !_me.isGuest}
    <div style="width:84px;height:84px;border-radius:24px;overflow:hidden;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center;margin-bottom:24px">
      {#if _me.avatarUrl}
    ...
    <h1 style="font-size:1.4rem;font-weight:700;color:#FFFFFF;margin:0 0 6px">{_me.name || 'Akun Ganify'}</h1>
    {#if _me.email}
      <p style="font-size:.82rem;color:rgba(255,255,255,.4);margin:0 0 32px">{_me.email}</p>
    {:else}
      <p style="font-size:.82rem;color:rgba(255,255,255,.4);margin:0 0 32px">Akun tertaut dengan Google</p>
    {/if}
  ...
  {:else}
    ...
    <p style="font-size:.82rem;color:rgba(255,255,255,.4);line-height:1.6;margin:0 0 32px;max-width:340px">Kamu masih pakai akun tamu. Hubungkan dengan Google biar data lagu, playlist, dan riwayat aman kalau ganti perangkat.</p>
